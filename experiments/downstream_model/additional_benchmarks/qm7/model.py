...
class SimpleGNN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        add = geom_nn.global_add_pool(x, batch)
        mean = geom_nn.global_mean_pool(x, batch)
        max = geom_nn.global_max_pool(x, batch)

        x = torch.cat([add, mean, max], dim=1)
        x = self.classifier(x)
...
class GNN(pl.LightningModule):
    def __init__(self, model_type, model_params, lr=2e-4):
        super().__init__()
        ...
        if model_type in ["GCN_tg", "GAT_tg", 'FiLM']:
            self.classifier = nn.Sequential(
                nn.Linear(model_params['out_channels']*3, 16),
                nn.Tanh(),
                nn.Linear(16, 1)
            )
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        if self.model_type in ['GCN_tg', 'FiLM']:
            ...
            # make all different pools
            add = geom_nn.global_add_pool(out, batch)
            mean = geom_nn.global_mean_pool(out, batch)
            max = geom_nn.global_max_pool(out, batch)
            out = torch.cat([add, mean, max], dim=1)
            ...
            out = geom_nn.global_add_pool(out, batch)
            out = self.classifier(out)
